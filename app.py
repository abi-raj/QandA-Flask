...
def index():
    ...
    db=get_db()
    db.execute('select id,question from questions join users on users.id=questions.asked_by_id join users on users.id=questions.expert_id where questions.answer is not null')
...
def unanswered():
    user = get_current_user()
    db=get_db()
    unanswered_cur = db.execute('select * from questions join users on users.id =questions.asked_by_id where questions.answer is null and questions.expert_id=?',[user['id']])
